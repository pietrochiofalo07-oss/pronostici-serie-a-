...
st.set_page_config(
    page_title="EUROPE AI PREDICTOR MINIMAL", 
    page_icon="⚡", 
    layout="centered"
)
...
st.markdown("### ⚡ MATCH DASHBOARD MINIMAL")
    ...
    if home_team == away_team:
        ...
        if st.button("ESEGUI ANALISI", type="primary", use_container_width=True):
            ...
            with st.container():
                ...
                with col1:
                    st.markdown(f"""
                    - **Esito 1X2:** `1` ({round(home_win, 1)}%) | `X` ({round(draw, 1)}%) | `2` ({round(away_win, 1)}%)
                    - **Combo Gol:** `OVER 2.5` ({round(prob_over25, 1)}%)
                    - **Entrambe a Segno:** `GOAL` ({round(prob_gg, 1)}%)
                    """)
                with col2:
                    st.markdown(f"""
                    - **Expected Goals (xG):** `{round(home_xg, 2)}` — `{round(away_xg, 2)}`
                    - **Angoli Previsti:** `{expected_corners}`
                    - **Cartellini Previsti:** `{expected_cards}`
                    """)
            ...
            with st.container():
                ...
                with col_m1:
                    ...
                    st.text(f"• Win Rate Totale: {h_data['win_rate']}%")
                    st.text(f"• Clean Sheet Prob: {h_data['clean_sheets_prob']}%")
                    st.text(f"• Forma Recente: {h_data['form_sequence']}")
                with col_m2:
                    st.markdown(f"**✈️ {away_team}**")
                    st.text(f"• Win Rate Esterno: {a_data['away_win_rate']}%")
                    st.text(f"• Clean Sheet Prob: {a_data['clean_sheets_prob']}%")
                    st.text(f"• Forma Recente: {a_data['form_sequence']}")
